...
// In-memory caches (note: reset on cold starts)
...
const ISS_CACHE_DURATION = 10 * 1000;
const ASTRO_CACHE_DURATION = 30 * 60 * 1000;
...
app.get("/api/astros", async (_req, res) => {
...
export const handler = async (event: any, context: any) => {
  try {
    ...
    console.error("Critical Function Relay Error:", error);
    return {
      statusCode: 503,
      body: JSON.stringify({
        error: "Station Offline",
        message: "The API relay station encountered a critical failure.",
